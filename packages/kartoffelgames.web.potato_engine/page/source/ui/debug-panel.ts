...
const gSystemColors: Array<string> = [
    '#89b4fa', // blue
    '#a6e3a1', // green
    '#f9e2af', // yellow
    '#cba6f7', // mauve
...
export class DebugPanel {
    ...
    private createColorControl(pContainer: HTMLElement, pTarget: object, pName: string): void {
        ...
        const lGetColor = (): Color => (pTarget as Record<string, Color>)[pName];

        // Helper: Convert color to hex and update swatch + picker.
        const lSyncPickerFromColor = (): void => {
            const lColor: Color = lGetColor();
            const lHex: string = '#' +
    ...
    private drawSparkline(pCanvas: HTMLCanvasElement, pSystemName: string, pHistory: ReadonlyArray<GameEnvironmentTimingSnapshot>): void {
        ...
        const lColor: string = gSystemColors[lSystemIndex % gSystemColors.length];
