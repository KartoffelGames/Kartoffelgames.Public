...
.selection-box {\r
    ...
    pointer-events: none;\r
    z-index: 1000;\r
}`;var Ai=`<!-- Serves only as a background. -->\r
...
return ${c};`)();return{type:e.documentPort.resolvedDataType,execute:()=>y(...m)}}});Pt.preview.addDisplay(new Ne(ys));Pt.preview.addDisplay(new Ne(bs));Pt.preview.addDisplay(new Se(ys));Pt.preview.addDisplay(new Se(bs));var Ra=document.getElementById("application-root"),_e=new oo(Pt);_e.appendTo(Ra);_e.document=new zt(Pt);ws();function ws(){try{_e.update()}catch(g){}requestAnimationFrame(ws)}document.getElementById("load-button").addEventListener("click",Oa);document.getElementById("save-button").addEventListener("click",Fa);var xs="potatno-code-document.json";async function Oa(){if(window.confirm("Load saved document?"))try{let r=await(await(await navigator.storage.getDirectory()).getFileHandle(xs)).getFile();_e.load(await r.text())}catch{window.alert("Could not load document.")}}async function Fa(){if(window.confirm("Override saved document?"))try{let r=await(await(await navigator.storage.getDirectory()).getFileHandle(xs,{create:!0})).createWritable();await r.write(_e.save()),await r.close()}catch{window.alert("Could not save document.")}}})();
