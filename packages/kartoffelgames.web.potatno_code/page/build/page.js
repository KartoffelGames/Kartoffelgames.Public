...
.panel-left {
    ...
    border-right: 1px solid var(--pn-border-default);
    display: flex;
...
.resize-handle-left:hover {
    background: var(--pn-accent-primary);
}
...
.panel-right {
    ...
    max-width: var(--pn-panel-max-width);
    background: var(--pn-bg-secondary);
    border-left: 1px solid var(--pn-border-default);
    display: flex;
    flex-direction: column;
    overflow: hidden;
    flex-shrink: 0;
...
`});var Ts,bs=p(()=>{Ts=`<div class="editor-layout">
    <div #panelLeft class="panel-left">
        <potatno-panel-left
            [nodeDefinitions]="this.nodeDefinitionList"
            [functions]="this.functionList"
            [activeFunctionId]="this.activeFunctionId"
            (node-drag-start)="this.onNodeDragFromLibrary($event)"
            (function-select)="this.onFunctionSelect($event)"
            (function-add)="this.onFunctionAdd()"
    ...
    <div class="center-area">
        <div #canvasWrapper class="canvas-wrapper"
            ...
            (pointermove)="this.onCanvasPointerMove($event)"
            (pointerup)="this.onCanvasPointerUp($event)"
            ...
            <div #gridLayer class="grid-layer" [style]="this.gridTransformStyle">
                ...
                <div class="node-layer">
                    $for(nodeRender of this.visibleNodes) {
                        <div class="node-position" style="left:{{this.nodeRender.pixelX}}px;top:{{this.nodeRender.pixelY}}px;width:{{this.nodeRender.size.w * this.interaction.gridSize}}px">
                            <potatno-node
                                [nodeData]="this.nodeRender"
                                [selected]="this.nodeRender.selected"
                                [gridSize]="this.interaction.gridSize"
                                (pointerdown)="this.onNodePointerDown($event, this.nodeRender)"
                                (port-drag-start)="this.onPortDragStart($event)"
                                (port-hover)="this.onPortHover($event)"
                                (port-leave)="this.onPortLeave($event)"
                                (resize-start)="this.onNodeResizeStart($event, this.nodeRender)"
                                (comment-change)="this.onCommentChange($event)"
                                (value-change)="this.onValueChange($event)">
                            </potatno-node>
                        </div>
                    }
                </div>
        ...
        $if(this.hasPreview) {
            <div class="preview-wrapper">
                <potatno-preview #previewEl [errors]="this.editorErrors"></potatno-preview>
            </div>
        }
    </div>
...
.search-wrapper:focus-within {
    border-color: var(--pn-accent-primary);
}
...
.category-list::-webkit-scrollbar-track {
    background: var(--pn-scrollbar-track);
}
...
`});var Us,js=p(()=>{Us=`<div class="search-wrapper">
    <span class="search-icon">\u2315</span>
    <input class="search-field" type="text" placeholder="Search nodes..." (input)="this.onSearchInput($event)" />
</div>
<div class="category-list">
    $for(group of this.filteredGroups) {
        <div class="category-group">
            <button class="category-header" (click)="this.toggleCategory(this.group.category)">
                <span class="category-border" style="background: {{this.group.cssColor}}"></span>
                <span class="category-icon">{{this.group.icon}}</span>
                <span class="category-label">{{this.group.label}}</span>
                <span [className]="this.getToggleClass(this.group.category)">\u25BC</span>
            </button>
            $if(!this.isCategoryCollapsed(this.group.category)) {
                <div class="category-items">
                    $for(node of this.group.nodes) {
                        <div class="node-entry" (mousedown)="this.onNodeMouseDown(this.node.name)">{{this.node.name}}</div>
                    }
                </div>
            }
        </div>
...
.function-entry.active {
    background: var(--pn-bg-surface);
    border-left: 2px solid var(--pn-accent-primary);
    padding-left: 10px;
}

.function-icon {
    font-size: var(--pn-font-size);
    color: var(--pn-cat-function);
    flex-shrink: 0;
    width: 16px;
    text-align: center;
}
...
.delete-button:hover {
    background: var(--pn-accent-danger);
    color: var(--pn-text-primary);
}
...
.name-input:invalid {
    border-color: var(--pn-accent-danger);
    outline-color: var(--pn-accent-danger);
}
...
.port-name-input:invalid {
    border-color: var(--pn-accent-danger);
    outline-color: var(--pn-accent-danger);
}

.port-name-input:disabled {
    color: var(--pn-text-muted);
    background: var(--pn-bg-secondary);
    cursor: not-allowed;
}
...
.port-type-input:focus {
    border-color: var(--pn-accent-primary);
}

.port-type-input:disabled {
    color: var(--pn-text-muted);
    background: var(--pn-bg-secondary);
    cursor: not-allowed;
}
...
.add-import-row .import-select {
    flex: 1;
}

.add-import-row .add-button {
    flex-shrink: 0;
    width: auto;
    margin-top: 0;
}
...
`});var va,Ta=p(()=>{va=`<div class="properties-header">Properties</div>
...
.preview-container {
    ...
    box-shadow: 0 4px 12px var(--pn-node-shadow);
    overflow: hidden;
}
...
.resize-handle::before {
    ...
    width: 8px;
    height: 8px;
    border-top: 2px solid var(--pn-text-muted);
    border-left: 2px solid var(--pn-text-muted);
    transition: border-color 0.15s;
}
...
.error-title {
    color: var(--pn-accent-danger) !important;
}
...
`});var Fa,Oa=p(()=>{Fa=`<div class="resize-handle" (pointerdown)="this.onResizePointerDown($event)"></div>
...
`});var Wa,Xa=p(()=>{Wa=`<div #viewport class="viewport" (pointerdown)="this.onPointerDown($event)" (pointermove)="this.onPointerMove($event)" (pointerup)="this.onPointerUp($event)" (wheel)="this.onWheel($event)" (contextmenu)="this.onContextMenu($event)" (keydown)="this.onKeyDown($event)" tabindex="0">
    <div #grid class="grid" [style]="this.gridStyle">
        <svg #svgLayer class="svg-layer" xmlns="http://www.w3.org/2000/svg"></svg>
        <div class="node-layer">
            $slot
        </div>
...
.node.selected {
    border-color: var(--pn-node-border-selected);
    box-shadow: 0 0 0 1px var(--pn-node-border-selected), 0 2px 8px var(--pn-node-shadow);
}
...
.node-value-row .node-value-input {
    flex: 1;
}

.node-value-row .node-outputs {
    flex-shrink: 0;
    margin-right: -8px;
}
...
.node-value-input input:focus {
    border-color: var(--pn-border-active);
}
...
.node-comment {
    ...
    border: 1px dashed var(--pn-cat-comment);
    border-radius: var(--pn-node-border-radius);
    min-width: 120px;
    min-height: 60px;
    position: relative;
...
.resize-handle::after {
    ...
    width: 8px;
    height: 8px;
    border-right: 2px solid var(--pn-text-muted);
    border-bottom: 2px solid var(--pn-text-muted);
    opacity: 0.5;
}
...
`});var pl,ul=p(()=>{pl=`$if(this.nodeData) {
    $if(this.isComment) {
        <div class="node-comment {{this.selectedClass}}"
             [style]="this.commentSizeStyle"
             (pointerdown)="this.onNodePointerDown($event)">
            <div class="comment-header">
                <span class="node-icon">{{this.nodeData.categoryIcon}}</span>
                <span class="node-label">{{this.nodeData.definitionName}}</span>
            </div>
            <div class="comment-body">
                <textarea [value]="this.nodeData.commentText"
    ...
    $if(!this.isComment) {
        <div class="node {{this.selectedClass}}"
             (pointerdown)="this.onNodePointerDown($event)">
            <div class="node-header" style="background: {{this.nodeData.categoryColor}}">
                <div class="header-flow-inputs">
                    $for(flowIn of this.flowInputPorts) {
                        <potatno-port
                            [name]="this.flowIn.name"
                            [portId]="this.flowIn.id"
                            [nodeId]="this.nodeId"
                            [direction]="'input'"
                            [portKind]="'flow'"
                            [connected]="this.flowIn.connectedTo !== null"
                            (port-drag-start)="this.onPortDragStart($event)"
                            (port-hover)="this.onPortHover($event)"
                            (port-leave)="this.onPortLeave($event)">
                        </potatno-port>
                ...
                <div class="header-flow-outputs">
                    $for(flowOut of this.flowOutputPorts) {
                        <potatno-port
                            [name]="this.flowOut.name"
                            [portId]="this.flowOut.id"
                            [nodeId]="this.nodeId"
                            [direction]="'output'"
                            [portKind]="'flow'"
                            [connected]="this.flowOut.connectedTo !== null"
                            (port-drag-start)="this.onPortDragStart($event)"
                            (port-hover)="this.onPortHover($event)"
                            (port-leave)="this.onPortLeave($event)">
                        </potatno-port>
            ...
            <div class="node-body">
                $if(this.isValue) {
                    <div class="node-value-row">
                        <div class="node-value-input">
                            <input type="text"
                                   [value]="this.nodeData.valueText"
                                   (input)="this.onValueInput($event)"
                                   (pointerdown)="$event.stopPropagation()"/>
                        </div>
                        <div class="node-outputs">
                            $for(outPort of this.outputPorts) {
                                <potatno-port
                                    [name]="this.outPort.name"
                                    [type]="this.outPort.type"
                                    [portId]="this.outPort.id"
                                    [nodeId]="this.nodeId"
                                    [direction]="'output'"
                                    [portKind]="'data'"
                                    [connected]="this.outPort.connectedTo !== null"
                                    (port-drag-start)="this.onPortDragStart($event)"
                                    (port-hover)="this.onPortHover($event)"
                                    (port-leave)="this.onPortLeave($event)">
                                </potatno-port>
                ...
                $if(!this.isValue) {
                    <div class="node-inputs">
                        $for(inPort of this.inputPorts) {
                            <potatno-port
                                [name]="this.inPort.name"
                                [type]="this.inPort.type"
                                [portId]="this.inPort.id"
                                [nodeId]="this.nodeId"
                                [direction]="'input'"
                                [portKind]="'data'"
                                [connected]="this.inPort.connectedTo !== null"
                                (port-drag-start)="this.onPortDragStart($event)"
                                (port-hover)="this.onPortHover($event)"
                                (port-leave)="this.onPortLeave($event)">
                            </potatno-port>
...
.port-circle.direction-output::before {
    left: 0;
    border-radius: 3px 0 0 3px;
}
.port-circle.direction-output::after {
    left: 9px;
    border-left: 7px solid var(--port-color, var(--pn-text-muted));
}

/* Input: arrow points left */
.port-circle.direction-input::before {
    right: 0;
    border-radius: 0 3px 3px 0;
}
.port-circle.direction-input::after {
    right: 9px;
    border-right: 7px solid var(--port-color, var(--pn-text-muted));
}

/* Connected: solid fill (default) */
.port-circle.connected::before { background: var(--port-color, var(--pn-text-muted)); }
.port-circle.connected.direction-output::after { border-left-color: var(--port-color, var(--pn-text-muted)); }
.port-circle.connected.direction-input::after { border-right-color: var(--port-color, var(--pn-text-muted)); }

/* Disconnected: dimmed */
.port-circle.disconnected::before { background: color-mix(in srgb, var(--port-color, var(--pn-text-muted)) 30%, var(--pn-node-bg)); }
.port-circle.disconnected.direction-output::after { border-left-color: color-mix(in srgb, var(--port-color, var(--pn-text-muted)) 30%, var(--pn-node-bg)); }
.port-circle.disconnected.direction-input::after { border-right-color: color-mix(in srgb, var(--port-color, var(--pn-text-muted)) 30%, var(--pn-node-bg)); }

/* Invalid: red glow */
.port-circle.invalid::before { background: var(--pn-accent-danger); }
.port-circle.invalid.direction-output::after { border-left-color: var(--pn-accent-danger); }
.port-circle.invalid.direction-input::after { border-right-color: var(--pn-accent-danger); }
...
`});var yl,hl=p(()=>{yl=`<div [className]="this.portWrapperClasses" [title]="this.type">
...
`});var mc,dc=p(()=>{mc=`<div class="search-wrapper">
    ...
    <input class="search-field" type="text" [placeholder]="this.placeholder" [value]="this.value" (input)="this.onInput($event)"/>
</div>
...
`),Oe.style.color="#cdd6f4"):(Oe.textContent="(no output)",Oe.style.color="#6c7086")}catch(e){Oe.textContent=`Error: ${e.message??e}`,Oe.style.color="#f38ba8"}});Fc=new mr(B);Fc.appendTo(document.body);Fc.file=new Wt});(()=>{let s=new WebSocket("ws://127.0.0.1:8088");s.addEventListener("open",()=>{console.log("Refresh connection established")}),s.addEventListener("message",e=>{console.log("Bundle finished. Start refresh"),e.data==="REFRESH"&&window.location.reload()})})();Promise.resolve().then(()=>Bc());
