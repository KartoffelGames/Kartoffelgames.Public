...
export class LogicalExpressionAst extends AbstractSyntaxTree<LogicalExpressionCst, LogicalExpressionAstData> implements IExpressionAst {
    ...
    protected override process(pContext: AbstractSyntaxTreeContext): LogicalExpressionAstData {
        ...
        if (!lLeftExpression.data.returnType.isImplicitCastableInto(new PgslBooleanType(pContext))) {
            pContext.pushIncident('Left side of logical expression needs to be a boolean', this);
        }

        // Validate right side type.
        if (!lRightExpression.data.returnType.isImplicitCastableInto(new PgslBooleanType(pContext))) {
        ...
        return {
            ...
            isStorage: false,
            returnType: new PgslBooleanType(pContext),
            constantValue: null,
