...
export class PgslSimpleType {
    ...
    public constructor(pTypeName: string, ...pGenerics: Array<PgslType>) {
        // Validate type.
        if (!PgslSimpleType.mTypeStorage.get(pTypeName)) {
            throw new Exception(`Invalid "${pTypeName}"`, this);
        }

        // Get type definition.
        const lTypeDefinition: TypeInformation = PgslSimpleType.mTypeStorage.get(pTypeName)!;
