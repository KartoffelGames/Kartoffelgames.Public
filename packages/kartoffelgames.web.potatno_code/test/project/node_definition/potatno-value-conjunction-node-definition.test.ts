...
Deno.test('ValueConjunctionNodeDefinition.DEFINITION_ID', async (pContext) => {
    await pContext.step('Returns stable definition id', () => {
        // Setup. Process.
        const lResult: string = PotatnoValueConjunctionNodeDefinition.DEFINITION_ID;
...
Deno.test('new ValueConjunctionNodeDefinition()', async (pContext) => {
    await pContext.step('Constructs value conjunction ports', () => {
        // Setup. Process.
        const lDefinition = new PotatnoValueConjunctionNodeDefinition<PotatnoTestProjectTypesDefinition>();

        // Evaluation.
        expect(lDefinition.id).toBe(PotatnoValueConjunctionNodeDefinition.DEFINITION_ID);
