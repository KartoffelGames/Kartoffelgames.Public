...
return ${a};`)();return{type:e.documentPort.resolvedDataType,execute:()=>y(...m)}}});Et.preview.addDisplay(new De(hi));Et.preview.addDisplay(new De(di));Et.preview.addDisplay(new Ee(hi));Et.preview.addDisplay(new Ee(di));var Is=document.getElementById("application-root"),Ie=new Ye(Et);Ie.appendTo(Is);Ie.document=new At(Et);mi();async function mi(){try{await Ie.update()}catch(f){}requestAnimationFrame(mi)}document.getElementById("load-button").addEventListener("click",Cs);document.getElementById("save-button").addEventListener("click",Ss);var fi="potatno-code-document.json";async function Cs(){if(window.confirm("Load saved document?"))try{let o=await(await(await navigator.storage.getDirectory()).getFileHandle(fi)).getFile();Ie.load(await o.text())}catch{window.alert("Could not load document.")}}async function Ss(){if(window.confirm("Override saved document?"))try{let o=await(await(await navigator.storage.getDirectory()).getFileHandle(fi,{create:!0})).createWritable();await o.write(Ie.save()),await o.close()}catch{window.alert("Could not save document.")}}})();
